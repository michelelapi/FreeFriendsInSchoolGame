    ...
    private void generateLayout() {
        ...
        for (int row = 0; row < roomsPerCol; row++) {
            for (int col = 0; col < roomsPerRow; col++) {
                ...
                if (row > 0) {
                    // Left segment
                    walls.add(new Wall(roomX, roomY, doorCenterX - doorWidth / 2 - roomX, WALL_THICKNESS, spriteManager));
                    // Right segment
                ...
                if (row < roomsPerCol - 1) {
                    ...
                    walls.add(new Wall(roomX, roomY + ROOM_SIZE - WALL_THICKNESS, ROOM_SIZE, WALL_THICKNESS, spriteManager));
                }
                
                // Left wall (with doorway if not first column)
                if (col > 0) {
                    // Top segment
                    walls.add(new Wall(roomX, roomY, WALL_THICKNESS, doorCenterY - doorWidth / 2 - roomY, spriteManager));
                    // Bottom segment
                ...
                if (col < roomsPerRow - 1) {
                    ...
                    walls.add(new Wall(roomX + ROOM_SIZE - WALL_THICKNESS, roomY, WALL_THICKNESS, ROOM_SIZE, spriteManager));
                }
            }
        }
